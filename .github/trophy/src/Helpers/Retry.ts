...
async function* createAsyncIterable<T>(
  ...
  for (let i = 0; i < retries; i++) {
    ...
    try {
      ...
      console.log("CAI_CAUGHT " + JSON.stringify({name: e?.name, message: e?.message, stack: e?.stack, isSoxa: e?.isSoxaError, status: e?.response?.status, data: e?.response?.data}, null, 1));
