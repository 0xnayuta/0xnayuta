...
export class GithubApiService extends GithubRepository {
  ...
  async requestUserInfo(username: string): Promise<UserInfo | ServiceError> {
    ...
    try {
      ...
      console.log("RUI_CAUGHT " + JSON.stringify({name: e?.name, message: e?.message, stack: e?.stack, isSoxa: e?.isSoxaError, status: e?.response?.status, data: e?.response?.data}, null, 1));
  ...
  async executeQuery<T = unknown>(
    ...
    try {
      ...
      console.log("EQ_CAUGHT " + JSON.stringify({name: e?.name, message: e?.message, stack: e?.stack, isSoxa: e?.isSoxaError, status: e?.response?.status, data: e?.response?.data}, null, 1));
